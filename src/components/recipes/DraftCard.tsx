      ...
      <CardContent className="flex items-center justify-between p-3">
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium">
            {draft.recipeJson.title || "Sin título"}
          </p>
          <p className="truncate text-xs text-zinc-500 dark:text-zinc-400">
            &quot;{draft.prompt}&quot; ·{" "}
            {new Date(draft.createdAt).toLocaleDateString("es-CO", {
              day: "numeric",
              month: "short",
            })}
          </p>
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={() => onView(draft.recipeJson)}
          ...
          <button
            onClick={() => onSave(draft.recipeJson)}
            className="rounded p-1.5 hover:bg-orange-100 dark:hover:bg-orange-900/30"
            title="Guardar"
          >
