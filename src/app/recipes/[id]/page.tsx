...
export default function RecipeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="mx-auto max-w-md md:max-w-2xl px-4 py-6 pb-24"><div className="space-y-3">{[1, 2, 3, 4].map((i) => <div key={i} className="h-20 animate-pulse rounded-xl bg-zinc-100 dark:bg-zinc-800" />)}</div></div>;
  if (!recipe) return <div className="mx-auto max-w-md md:max-w-2xl px-4 py-6 pb-24 text-center"><p className="text-zinc-500">{t("detail.not_found")}</p></div>;
